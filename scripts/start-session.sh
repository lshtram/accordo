...
usage() {
  cat <<'EOF'
Usage: scripts/start-session.sh [options] [project-path]
...
Examples:
  ...
  scripts/start-session.sh /path/to/my-extension-project

  # Open with explicit package directories
  scripts/start-session.sh /path/to/project --pkg src/extensions/alpha --pkg src/extensions/beta
...
if [[ ${#PKG_DIRS[@]} -eq 0 ]]; then
  echo "[start-session] Auto-discovering VS Code extensions from: $ACCORDO_PKG_DIR"
