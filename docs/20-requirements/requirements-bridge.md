...
| LCM-13 | When a registry entry exists for `projectId`, Bridge MUST probe the registry-recorded port explicitly and parse the full `/health` JSON payload (not only HTTP 200/timeout) before deciding to reuse that Hub. The probing contract is port-explicit end-to-end: registry probe supplies `port`, health reader consumes `port`. |
| LCM-14 | Bridge MUST classify registry probe outcomes using deterministic IDs: `registry-missing`, `registry-stale`, `registry-unreachable`, `registry-empty`, `registry-loaded`, `bridge-connected`. Outcome meaning: `registry-loaded` = `/health` ok with `bridge:"disconnected"` and `toolCount > 0`; `registry-empty` = `/health` ok with `bridge:"disconnected"` and `toolCount === 0`; `bridge-connected` = `/health` ok with `bridge:"connected"` regardless of `toolCount`. |
| LCM-15 | Reuse decision: Bridge MAY reuse only `registry-loaded` and `bridge-connected` outcomes. It MUST treat `registry-empty`, `registry-stale`, `registry-unreachable`, and `registry-missing` as non-reusable and continue to spawn/hard-recovery flow. |
| LCM-16 | Bridge MUST emit a structured diagnostic record for every startup probe outcome containing at minimum `{ projectId, registryPath, outcome, port?, pid?, bridge?, toolCount? }`; `protocolVersion?` MAY also be included. Code interfaces MUST expose these fields explicitly rather than only nested entry/health objects. Diagnostics MUST NOT include bearer tokens, bridge secrets, or raw SecretStorage values. |
