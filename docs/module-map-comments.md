...
`comments-bootstrap.ts` — The real composition root: creates CommentStore, loads persisted data, initialises NativeComments (gutter, panel, inline threads), wires commands, registers Bridge tools, and publishes modality state.
...
| `panel-bootstrap.ts` | Creates the custom Comments webview panel and registers user-facing commands | `wirePanelAndCommands()` |
...
- **Bridge tools**: Four MCP tools (`comment_create`, `comment_reply`, `comment_resolve`, `comment_delete`, `comment_list`, `comment_sync_version`) are registered with the Bridge when accordo-bridge is present.
...
- **`panel-bootstrap.ts`** is internal to the bootstrap layer — it creates VSCode webview and command infrastructure that other packages do not need to know about.
