...
**References:**
- `docs/patterns.md` P-12 — root-cause analysis of native Comments panel limitations
- `docs/architecture.md` §12.4 — architectural decision record
- `docs/workplan.md` deferred backlog item #7
- `docs/requirements-comments-panel.md` — module specifications and requirement IDs
...
The built-in panel does **not** honour `view/item/context` menu contributions from extensions. That contribution point only works on extension-contributed `TreeView` instances (see `docs/patterns.md` P-12). The only action available in the built-in panel is VS Code's own "Reply" entry, and only when `widget.canReply` is truthy.
