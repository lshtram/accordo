...
| `shared-relay-server.ts` | Public export surface for the shared relay server | `SharedBrowserRelayServer` class |
| `shared-relay-server-runtime.ts` | Shared relay server composition/runtime; owns HTTP + WS surfaces and delegates chrome/hub wiring | `SharedBrowserRelayServer` class |
| `shared-relay-server-http.ts` | Pairing HTTP server endpoint creation | `createSharedRelayHttpServer()` |
| `shared-relay-server-hub.ts` | Hub connection routing, request forwarding, and reconnect metadata | `attachHubConnection()` |
| `shared-relay-server-chrome.ts` | Chrome connection lifecycle and broadcast wiring | `attachChromeConnection()` |
...
| `page-tool-handlers-impl.ts` | Legacy compatibility barrel for page-understanding handlers | Re-exports current handler implementations |
| `page-tool-page-map-handler.ts` | `get_page_map` handler using the shared page-tool pipeline | `handleGetPageMap()` |
| `page-tool-inspect-handler.ts` | `inspect_element` handler using the shared page-tool pipeline | `handleInspectElement()` |
| `page-tool-dom-excerpt-handler.ts` | `get_dom_excerpt` handler using the shared page-tool pipeline | `handleGetDomExcerpt()` |
| `text-map-tool-handler.ts` | `get_text_map` handler using the shared page-tool pipeline plus pagination | `handleGetTextMap()` |
| `page-tool-capture-handler.ts` | `capture_region` handler with screenshot-store and transport logic | `handleCaptureRegion()` |
| `page-tool-pipeline.ts` | Shared page-tool flow for audit/origin/snapshot/redaction/post-process behavior | `runPageToolPipeline()` |
...
| `snapshot-retention.ts` | 10-slot per-page FIFO snapshot retention store for page-understanding tools | `SnapshotRetentionStore` class |
...
- **`page-tool-handlers-impl.ts`** is a compatibility surface only — prefer the barrel `page-tool-handlers.ts` or the focused handler modules.
