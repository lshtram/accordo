...
2. **Hub serves MCP at `POST /mcp` plus notification SSE at `GET /mcp`.** JSON-RPC requests are sent to `POST /mcp`; server-initiated notifications (for example `notifications/tools/list_changed`) use authenticated SSE on `GET /mcp`.
...
| `/browser/status` | GET | Returns browser relay status `{ connected, controlGranted }` for browser tooling diagnostics. |

### 3.4 MCP Transport — Streamable HTTP

Per MCP spec 2025-03-26:

- **Endpoints:** `POST /mcp` for JSON-RPC request/response and `GET /mcp` for SSE notifications
- **Request body (POST):** JSON-RPC 2.0 request or batch
- **Response (POST):** JSON-RPC 2.0 response
- **Response (GET):** `text/event-stream` notification channel
...
- **Heartbeat:** Hub sends `ping` every 30s on the active bridge socket. Current implementation does not enforce a missed-`pong` disconnect timeout.
...
│   ├── bridge-dispatch.ts   — Bridge message routing, invoke queue, pending-call tracking
...
| Parameter | Value |
|---|---|
| Hub → Bridge ping interval | 30 seconds |
| Pong grace window | Not enforced in current implementation |
| Bridge reconnect backoff | 1s, 2s, 4s, 8s, 16s, max 30s |
| Hub state hold window after disconnect | 10 seconds |
