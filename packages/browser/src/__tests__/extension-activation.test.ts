...
vi.mock("vscode", () => {
  ...
  return {
    ...
    createExtensionContextMock: () => ({
      ...
      _secretsStore: secretsStore, // test access
    }),
  };
});
...
describe("M83-BTOOLS extension activation", () => {
  ...
  describe("AUTH-lifecycle: SecretStorage primary token storage", () => {
    it("AUTH-03: SecretStorage has token → that token is used (no dev fallback)", async () => {
      const bridge = {
        registerTools: vi.fn().mockReturnValue({ dispose: vi.fn() }),
        publishState: vi.fn(),
        invokeTool: invokeToolMock,
      ...
      const PRE_STORED_TOKEN = "my-secret-storage-token-xyz789";
      const context = {
        subscriptions: [] as Array<{ dispose(): void }>,
        globalState: {
          get: vi.fn((_k: string) => undefined as unknown),
          update: vi.fn(async (_k: string, _v: unknown) => {}),
        },
        secrets: {
          get: vi.fn(async (_k: string) => PRE_STORED_TOKEN),
      ...
      // The token from SecretStorage must be used — no dev fallback.
      // The server was started with the stored token.
      // No globalState migration should have happened (SecretStorage hit on first step).
      expect(context.secrets.store).not.toHaveBeenCalled();
      expect(context.globalState.update).not.toHaveBeenCalled();
    });

    it("AUTH-03: SecretStorage absent, globalState has token → migration to SecretStorage", async () => {
      const bridge = {
        registerTools: vi.fn().mockReturnValue({ dispose: vi.fn() }),
        publishState: vi.fn(),
        invokeTool: invokeToolMock,
      ...
      const OLD_GLOBAL_STATE_TOKEN = "legacy-globalstate-token-abc";
      const context = {
        subscriptions: [] as Array<{ dispose(): void }>,
        globalState: {
          get: vi.fn((_k: string) => OLD_GLOBAL_STATE_TOKEN),
          update: vi.fn(async (_k: string, _v: unknown) => {}),
        },
        secrets: {
      ...
      // Token must be migrated: secrets.store called with the old globalState token.
      expect(context.secrets.store).toHaveBeenCalledWith("browserRelayToken", OLD_GLOBAL_STATE_TOKEN);
      // globalState should be cleaned up.
      expect(context.globalState.update).toHaveBeenCalledWith("browserRelayToken", undefined);
    });

    it("AUTH-03: neither store has token → fresh token generated and stored in SecretStorage", async () => {
      const bridge = {
        registerTools: vi.fn().mockReturnValue({ dispose: vi.fn() }),
        publishState: vi.fn(),
        invokeTool: invokeToolMock,
      ...
      // A fresh token must have been generated and stored in SecretStorage.
      // The token must be a non-empty UUID-formatted string (generated by generateRelayToken).
      const storeCall = (context.secrets.store as ReturnType<typeof vi.fn>).mock.calls.find(([k]) => k === "browserRelayToken");
      expect(storeCall).toBeDefined();
      const generatedToken = storeCall![1] as string;
      expect(generatedToken.length).toBeGreaterThan(10);
      // Token must NOT be the old hardcoded dev token.
      expect(generatedToken).not.toBe("accordo-local-dev-token");
      // globalState must not have been written with a stale token.
      const globalUpdate = (context.globalState.update as ReturnType<typeof vi.fn>).mock.calls.find(([k]) => k === "browserRelayToken");
      // globalState.update with undefined is cleanup (migration) — acceptable.
      // But it should NOT be set to a hardcoded string.
      if (globalUpdate) {
    ...
    it("AUTH-03-ERR: secrets.store throws during migration → keep global token, no cleanup", async () => {
      ...
      const OLD_TOKEN = "legacy-token-from-globalstate";
      const context = {
        subscriptions: [] as Array<{ dispose(): void }>,
        globalState: {
          get: vi.fn((_k: string) => OLD_TOKEN),
          update: vi.fn(async (_k: string, _v: unknown) => {}),
        },
        secrets: {
          get: vi.fn(async (_k: string) => undefined),
          store: vi.fn(async (_k: string, _v: string) => { throw new Error("keyring write failed"); }),
        },
      };

      await activate(context as never);

      // Migration store threw — must keep using globalState token.
      expect(context.secrets.store).toHaveBeenCalled();
      // globalState.update must NOT be called (no cleanup attempted after failed store)
      expect(context.globalState.update).not.toHaveBeenCalled();
    });

    it("AUTH-03-ERR: globalState.update throws after successful migration store", async () => {
      const bridge = {
        registerTools: vi.fn().mockReturnValue({ dispose: vi.fn() }),
        publishState: vi.fn(),
        invokeTool: invokeToolMock,
      ...
      const context = {
        ...
        globalState: {
          get: vi.fn((_k: string) => OLD_TOKEN),
          update: vi.fn(async (_k: string, _v: unknown) => {
            if (_v === undefined) throw new Error("globalState write error");
          }),
        },
        secrets: {
          get: vi.fn(async (_k: string) => undefined),
      ...
      // Migration store succeeded — cleanup threw, which is non-fatal.
      expect(context.secrets.store).toHaveBeenCalledWith("browserRelayToken", OLD_TOKEN);
      expect(context.globalState.update).toHaveBeenCalledWith("browserRelayToken", undefined);
    });
    ...
    it("AUTH-03-ERR: secrets.store throws for fresh token → ephemeral token returned", async () => {
      ...
      const context = {
        ...
        secrets: {
          get: vi.fn(async (_k: string) => undefined),
          store: vi.fn(async (_k: string, _v: string) => { throw new Error("keyring unavailable"); }),
        },
      };
