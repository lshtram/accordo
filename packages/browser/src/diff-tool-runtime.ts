...
export function buildTransientRelayError(topLevelError?: unknown): DiffToolError | undefined {
  if (topLevelError === "browser-not-connected") {
    return { success: false, error: "browser-not-connected", errorCode: "browser-not-connected", retryable: true, retryAfterMs: getRelayRetryAfterMs("browser-not-connected"), recoveryHints: getRelayRecoveryHint("browser-not-connected") };
  }
  if (topLevelError === "timeout") {
    return { success: false, error: "timeout", errorCode: "timeout", retryable: true, retryAfterMs: getRelayRetryAfterMs("timeout"), recoveryHints: getRelayRecoveryHint("timeout") };
  }
  return undefined;
}
