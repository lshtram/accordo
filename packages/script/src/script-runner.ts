 ...
 * All side-effectful VS Code operations are injected via ScriptRunnerDeps
...
export interface ScriptRunnerDeps {
   ...
   * Maps to vscode.commands.executeCommand in production.
   */
  ...
  /**
   * Show a subtitle string in the status bar for the given duration.
   */
...
export class ScriptRunner {
   ...
   */
  // Public overload — one argument only. Second arg (extraCallbacks) is intentionally
  // not in the public type; tests inject it via @ts-expect-error for wiring verification.
  run(script: NarrationScript): void;
  run(script: NarrationScript, extraCallbacks?: ScriptRunnerCallbacks): void {
    ...
    this._status = {
      ...
      label: script.label,
      scriptId: this._status.scriptId,
    };
