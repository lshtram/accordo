...
- For slide/Marp comments, use canonical slide coordinates, including the
  ...
  `slideIndex` is zero-based (`1` targets presentation slide 2); `x` and `y`
  are normalized 0..1 slide coordinates. Do not use pixel-only
...
- Delete comments carefully: delete a whole thread with `comment_delete({ threadId })`;
  ...
  Do not pass empty optional strings such as `commentId: ""`; omit the field.
- To start a clean comment session, use `comment_delete({ all: true })` and then
  verify with `comment_sync_version`.
